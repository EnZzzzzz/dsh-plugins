...
const WEIXIN_BRIDGE_SETTINGS_NAMESPACE = 'weixin-bridge';
/** Schema for the settings namespace; optionality is expressed in the TS type. */
export const WeixinBridgeSettingsSchema = z.object({
    provider: z.string(),
    model: z.string(),
    cwd: z.string(),
    maxTokens: z.natural(),
...
export function apply(ctx, config = {}) {
        ...
        try {
            ...
            if (accountId !== undefined) {
                ...
                if (holderPid !== undefined) {
                    ctx.logger.warn(`[weixin-bridge] 检测到另一个 DeepSeek Harness 实例（pid ${holderPid}）正在监听同一个微信账号，`
    ...
    ctx.effect(() => {
        ...
        ctx.inject(['connection', 'settings'], (apiCtx) => {
            const scope = apiCtx.settings.register(WEIXIN_BRIDGE_SETTINGS_NAMESPACE, WeixinBridgeSettingsSchema, {
            ...
            apiCtx.effect(() => scope.watch((next) => applyResolvedConfig(next)), 'weixin-bridge: settings watch');
