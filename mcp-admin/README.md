...
| `publicUrl` | string | 空 | 本部署公网/局域网访问地址（如 `http://1.2.3.4:3080`）的**初始值**。一般不用在这里配——直接在 Settings「MCP 管理」页编辑（持久化到 `settings.yaml`，热生效）。NAT 机器（腾讯云等）自己不知道公网 IP，走隧道打开 Web UI 时必配 |
