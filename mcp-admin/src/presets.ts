...
import { COMPOSITION_FILE, renderPresetMetadata, type AgentPresets } from '@deepseek-ai/dsh-agent-presets'
 ...
 */
export async function listPresets(presets: AgentPresets): Promise<PresetInfo[]> {
 ...
 */
export async function readPreset(presets: AgentPresets, id: string): Promise<string> {
  assertId(id)
...
export async function upsertPreset(
  ...
  if (!existsSync(target)) {
    ...
    if (existing !== undefined && existing.trust !== 'user' && base !== id) {
      throw new Error(
        `preset ${JSON.stringify(id)} exists under ${existing.trust} trust; pass base: ${JSON.stringify(id)} to copy it into the user root before editing`,
      )
    }
    if (base !== undefined) {
  ...
  if (metadata !== undefined) {
    ...
    if (rendered !== undefined) writeFileSync(join(presetRoot(), id, METADATA_FILE), rendered, 'utf8')
  }
 ...
 */
export async function deletePreset(presets: AgentPresets, id: string): Promise<void> {
  assertId(id)
