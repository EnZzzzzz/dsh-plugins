...
function parseFrontmatter(content: string): { name?: string; description?: string } {
  ...
  for (const line of match[1].split(/\r?\n/)) {
    ...
    if (field[1] === 'name') fields.name = value
    if (field[1] === 'description') fields.description = value
  }
  return fields
}

/** Resolve the markdown file for `name`, or undefined when absent from the user root. */
function locate(name: string): { form: 'bundle' | 'flat'; path: string } | undefined {
  const bundle = join(skillRoot(), name, 'SKILL.md')
  if (existsSync(bundle)) return { form: 'bundle', path: bundle }
  const flat = join(skillRoot(), `${name}.md`)
  if (existsSync(flat)) return { form: 'flat', path: flat }
  return undefined
}
 ...
 */
export function readSkill(name: string): { content: string; files: string[] } {
  assertName(name)
  const found = locate(name)
  if (!found) throw new Error(`skill ${JSON.stringify(name)} not found under ${skillRoot()}`)
 ...
 */
export function upsertSkill(name: string, content: string): string {
  assertName(name)
